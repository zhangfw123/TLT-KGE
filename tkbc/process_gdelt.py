...
def prepare_dataset(path, name):
    ...
    for f in files:
        ...
        for line in to_read.readlines():
            lhs, rel, rhs, timestamp = line.strip().replace(' ', '').split('\t')
    ...
    for f in files:
        ...
        examples = []
        for line in to_read.readlines():
            lhs, rel, rhs, ts = line.strip().replace(' ', '').split('\t')
            try:
                examples.append([entities_to_id[int(lhs)], relations_to_id[int(rel)], entities_to_id[int(rhs)], timestamps_to_id[ts]])
            except ValueError:
                continue
